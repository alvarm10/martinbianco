...
var gameattempts=[];
var gameattempts=[];
// CREO UN ARRAY VACIO PARA LOS RECURSOS CLICABLES DE LA ÚLTIMA PÁGINA VISITADA EN CASO DE NAVEGACION RESTRINGIDA
var clickmes = [];
var paginaBloqueada; //inicializar la variable que guarda que página se había bloqueado, para la navegtación restringida de académico
...
if (escormizar == "si"){
	...
	//SI YA HAY PUNTUACIONES E INTENTOS EN EL CURSO, LLENO LOS ARRAYS DE INTENTOS Y PUNTAJE CON SUS RESPECTIVOS VALORES
		...
		clickmes = obtenerdatos("clicames");
	}
}
...
clickmesjson=JSON.stringify(clickmes);
...
function cargar(){
    ...
    $('#btn-nav-phone, #btn-nav-phone2').on('click',function(){
...
function cargarInterfaz(){
	...
	$('title').html(nombreUnidad);
	recogerDatos();
	pintarSumario();
	navegarIndice();
	pintarIconos();
	//cambiarIdioma();
	if (lineaNegocio == "grupo") {
		$(".footer-grupo").css("display","block");
		$(".header-grupo")&&$(".header-desktop").removeClass("hidden-desktop").addClass("visible-desktop");
		$(".header-grupo")&&$(".header-phone").removeClass("hidden-tablet hidden-phone").addClass("visible-tablet visible-phone");
		$(".header-titulo-phone").removeClass("hidden-phone hidden-tablet").addClass("visible-phone visible-tablet");
		avanzarRetrocederTeclado();
	}
	if (lineaNegocio == "academico") {
		$(".header-academico").css("display","block");
		$(".header-titulo-academico").css("display","block");
		$(".container-fluid").addClass("margen-academico");
		$(".footer-academico").css("display","block");
		...
		$(".footer-phone .nav-control .nav-control-sh .fa-angle-double-left").click(function(){
		$(".footer-phone .nav-control .nav-control-sh .fa-angle-double-left").toggleClass("fa-rotate-90");
		$(".footer-phone .nav-control .nav-control-sh .fa-angle-double-left").toggleClass("fa-rotate-270");
		$(".footer-phone .nav-control .nav-control-sh .fa-angle-double-left").one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend',   
	    function(e) {
		    $(".footer-phone .botones").toggleClass("despliega");
			if ($(".icon-next").hasClass("sigueleyendo")) {$(".footer-phone .nav-control-bar").toggleClass("despliega");}
	    });
		})
		$(".footer-desktop .nav-control .nav-control-sh .fa-angle-double-left").click(function(){
			$(".footer-desktop .nav-control .nav-control-sh .fa-angle-double-left").toggleClass("fa-rotate-90");
			$(".footer-desktop .nav-control .nav-control-sh .fa-angle-double-left").toggleClass("fa-rotate-270");
			$(".footer-desktop .nav-control .nav-control-sh .fa-angle-double-left").one('webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend',   
		    function(e) {
			    $(".footer-desktop .nav-control-bar").toggleClass("despliega");
		    });
		})
		if (navegacion == "libre") {
			$(".footer-academico .footer-desktop").addClass("nav-control-oculto");
			avanzarRetrocederTeclado();
		}
		else {
			paginaBloqueada = $pagina;
			avanzarRetrocederTecladoAcademico()
		}
...
function pintarIconos(){
	...
	$('.modalayuda .modal-header > h4').html($ayuda+" "+tooltipayuda);
	$('#ModalGlosario .modal-header > h4').html($glosario+" "+tooltipglosario);
	$('#ModalDocs .modal-header > h4').html($docs+" "+tooltipdocumentacion);
}
...
function avanzarRetrocederTeclado(){	
	$(document).keydown(function(e) {
		//Cuando pulsa en la flecha de ir a la izquierda retrocede una página del curso (y cierra el lightbox si había una imagen ampliada abierta)
	    if(e.keyCode==37){
	    	retroceder();
	    	$("#lightbox").hide();
			$("#lightboxOverlay").hide();
	    }
	    //Cuando pulsa en la flecha de ir a la derecha avanza una página del curso (y cierra el lightbox si había una imagen ampliada abierta)
...
/* 
	** Function: avanzarRetrocederTecladoAcademina();
	** Description: Esta función similar a la anterior tiene las modificaciones necesarias para la navegación restringida de académico.
	============================================================================= */
function avanzarRetrocederTecladoAcademico(){
	$(document).keydown(function(e) {
		//Cuando pulsa en la flecha de ir a la izquierda retrocede una página del curso (y cierra el lightbox si había una imagen ampliada abierta)
	    if(e.keyCode==37){
	    	retroceder();
	    ...
	    //Cuando pulsa en la flecha de ir a la derecha avanza una página del curso (y cierra el lightbox si había una imagen ampliada abierta)
	    if(e.keyCode==39  && paginaBloqueada!=$pagina){
	    	avanzar();
	    	$("#lightbox").hide();
			$("#lightboxOverlay").hide();
	    }
...
function actualizarPagina(){
        if (escormizar == "si"){
	    	...
	    	doLMSSetValue('cmi.suspend_data', 'cont='+$pagina+'||array='+arrayestadosjson+'||intentosjuegos='+JSON.stringify(gameattempts)+'||puntosjuegos='+JSON.stringify(gamescores)+'||clicames='+JSON.stringify(clickmes));
		}
...
function obtenerdatos(data){
    ...
    //SI SE ACABA DE EMPEZAR EL CURSO
    if (_localLocation == "") {
            doLMSSetValue('cmi.suspend_data', 'cont='+1+'||array=["icon-no-checked","icon-no-checked","icon-no-checked","icon-no-checked","icon-no-checked"]||intentosjuegos='+gameattemptsjson+'||puntosjuegos='+gamescoresjson+'||clicames='+clickmesjson);
            _localLocation = doLMSGetValue("cmi.suspend_data");
        }
    ...
    switch (data) {
            ...
            case "intentosjuegos":
                ...
                break;
            case "puntosjuegos":
                parte4=resultado[3];//me devuelve array?con los puntos de cada juego
                puntosjuegos=(parte4.split("="))[1];
                respuntos = stripslashes(puntosjuegos)
                return JSON.parse(respuntos);
                break;
            case "clicames":
                parte5=resultado[4];//me devuelve array?con los recursos que se han pulsado o falta pulsar
                clicames=(parte5.split("="))[1];
                resclicks = stripslashes(clicames)
                return JSON.parse(resclicks);
                break;    
            default:
                //
                // Default
