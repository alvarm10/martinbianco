...
var respcorrecta;
var intentosrealizados = 0;//Variable para guardar los intentos realizados por el alumno en la parte del test
...
function cargarPagina(){
	...
	if(testTerminado == true && puntuacion > 98){
		pagina = 2;
	}
	...
	if($pagina > $paginaInicioTest-1){ //A paginas totales se le resta 1 porque la unidad en realidad tiene una pagina menos ya que la ultima pagina esta reservada para el test completo.
		if($pagina == $paginaInicioTest){
			...
			$('.banda3 h2').html(txtTituloTest);
			$('.banda3 h3').html("");	//Solo pongo titulo del test (sin subtitulo) puesto que ya estoy dentro del test.
		}
	}else{
		$pag = $pagina - 1;
